...
// ─────────────── STUDY ROADMAP ───────────────
const StudyRoadmap = ({ subjects, uid, studyPhases, storagePrefix, trackData, setTrackData }) => {
  const prefs = trackData?.prefs || null;
  const struckOutDays = trackData?.struckOutDays || {};
  const isTimetableView = trackData?.isTimetableView || false;
  const isCustomOrderMode = trackData?.isCustomOrderMode || false;
  ...
  const updateGlobalProgress = (updates) => {
    setTrackData(prev => ({
      ...(prev || {}),
      ...updates
    }));
  ...
  useEffect(() => {
    if (prefs && !prefs.customOrder && subjects && subjects.length > 0) {
      ...
      setQAnswers(p => ({ ...p, customOrder: order }));
      updateGlobalProgress({ prefs: { ...prefs, customOrder: order } });
    }
  }, [subjects, prefs]);
  ...
  const performSwap = (phaseId, idx1, idx2, currentSchedule) => {
      ...
      updateGlobalProgress({ prefs: newPrefs });
      setSwapSource(null);
  };
  ...
  const handleTopicDayChange = (subId, topic, currentVal, delta) => {
    ...
    updateGlobalProgress({ prefs: updatedPrefs });
  };

  const handleSave = () => {
    updateGlobalProgress({ prefs: qAnswers });
    setIsQuestionnaire(false);
  };
  ...
  const toggleTopic = (subId, topic, isChecked) => {
    const updateFn = prev => {
      ...
      return { ...prev, started: true, completedTopics: { ...prev.completedTopics, [subId]: updated } };
    };

    setQAnswers(updateFn);
    if (prefs) updateGlobalProgress({ prefs: updateFn(prefs) });
  };

  const selectAllTopics = (subId, topics, isFull) => {
    const finalTopics = isFull ? [] : [...topics];
    const updateFn = prev => ({
      ...prev,
      started: true,
      completedTopics: {
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
             ...
             <button className="btn btn-primary" style={{ width: '100%', background: struckOutDays[id] ? 'var(--rose)' : 'var(--emerald)', border: 'none', color: '#fff', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, padding: 12 }}
                onClick={() => {
                    if (struckOutDays[id]) {
                       const nextStruck = { ...struckOutDays };
                       delete nextStruck[id];
                       updateGlobalProgress({ struckOutDays: nextStruck });
                       toggleTopic(sub.id, topic, false);
                   } else {
                       const nextStruck = { ...struckOutDays, [id]: true };
                       updateGlobalProgress({ struckOutDays: nextStruck });
  ...
  if (!trackData) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '200px' }}>
        <p style={{ color: 'var(--text-muted)' }}>Waiting for track initialization...</p>
      </div>
    );
  }
  ...
  const moveSubject = (idx, direction) => {
    ...
    updateGlobalProgress({ prefs: { ...prefs, customOrder: newOrder } });
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16, gap: 12 }}>
        <button className={`btn btn-sm ${!isCustomOrderMode && !isTimetableView ? 'btn-primary' : 'btn-secondary'}`} onClick={() => updateGlobalProgress({ isCustomOrderMode: false, isTimetableView: false })}>
           AI Subject Phases
        </button>
        <button className={`btn btn-sm ${isCustomOrderMode ? 'btn-primary' : 'btn-secondary'}`} onClick={() => updateGlobalProgress({ isCustomOrderMode: true, isTimetableView: false })}>
           <ListOrdered size={14} /> Custom Order
        </button>
        <button className={`btn btn-sm ${isTimetableView ? 'btn-primary' : 'btn-secondary'}`} onClick={() => updateGlobalProgress({ isTimetableView: true, isCustomOrderMode: false })}>
          ...
          if (isTimetableView) {
             return phases.map((phase) => {
                 ...
                 phase.subjects.forEach(sub => {
                     // We don't filter topics here anymore to keep the shuffle stable!
                     const allTopics = sub.topics || [];
                     if (allTopics.length === 0) return;
                     
                     // Base completion is based on subjects, but we display all topics
                                        ...
                                        {weekDays.map((dayItem, localIdx) => {
                                            ...
                                            return (
                                                <div key={localIdx} style={{ 
                                                    ...
                                                    {!swapSource && (
                                                        <button className={`btn ${isStruckOut ? 'btn-danger' : 'btn-ghost'}`} 
                                                           style={{ 
                                                               padding: '3px 6px', 
                                                               borderRadius: 4, 
                                                               display: 'flex', 
                                                               ...
                                                               background: isStruckOut ? 'rgba(239, 68, 68, 0.15)' : 'var(--bg-tertiary)',
                                                               color: isStruckOut ? 'var(--rose)' : 'var(--text-muted)',
                                                               fontSize: 10,
                                                               gap: 4
                                                           }}
                                                           onClick={e => {
                                                           ...
                                                           {isStruckOut ? <><RotateCcw size={12} /> Undo</> : <Check size={14} />}
                                                        </button>
                                                    )}
...
// ─────────────── MOCK QUIZ ───────────────
const MockQuiz = ({ subjects, uid, storagePrefix, trackData, updateGlobalProgress }) => {
  ...
  const startQuiz = useCallback((sub) => {
    ...
    setQIdx(0);
    setAnswers({});
    setTimeLeft(sub.questions.length * 60); // 1 min per question
    setPhase('active');
    setShowExplanation({});
  }, []);
  ...
  const handleSubmit = useCallback(() => {
    ...
    // Save to centralized trackData (DB)
    const history = trackData?.quizHistory || [];
    updateGlobalProgress({ quizHistory: [...history, result] });
    setPhase('result');
  }, [selectedSubject, answers, trackData, updateGlobalProgress]);
...
// ─────────────── PERFORMANCE ───────────────
const PerformanceView = ({ subjects, trackData }) => {
  const history = trackData?.quizHistory || [];
...
const StudyTrackModule = ({ onBack, examInfo, platforms, branches, studyPhases, storagePrefix }) => {
  ...
  // Track-wide state loaded from backend or localStorage
  const [branch, setBranch] = useState(branchOptions[0]?.code);
  const [activeTab, setActiveTab] = useState('overview');
  const [trackData, setTrackData] = useState(null);
  const [loading, setLoading] = useState(true);

  // Load track state from backend on mount
  useEffect(() => {
    const fetchTrackState = async () => {
      try {
        const token = localStorage.getItem('vm_token');
        const res = await axios.get(`http://localhost:8000/api/study-progress/${storagePrefix}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        
        if (res.data.progress_data) {
          const data = res.data.progress_data;
          
          // Legacy migration for Quizzes
          const key = storageKey(uid, storagePrefix, 'quiz_history');
          const localHistory = JSON.parse(localStorage.getItem(key) || '[]');
          if (localHistory.length > 0 && (!data.quizHistory || data.quizHistory.length === 0)) {
            data.quizHistory = localHistory;
            localStorage.removeItem(key); // Cleanup
          }

          setTrackData(data);
          if (data.branch && branches[data.branch]) setBranch(data.branch);
          if (data.activeTab) setActiveTab(data.activeTab);
        } else {
          // Fallback to localStorage
          const savedBranch = localStorage.getItem(`${storagePrefix}_branch_${uid}`);
          const savedTab = localStorage.getItem(`${storagePrefix}_tab_${uid}`);
          const savedPrefsRaw = localStorage.getItem(storageKey(uid, storagePrefix, 'roadmap_prefs'));
          const savedPrefs = savedPrefsRaw ? JSON.parse(savedPrefsRaw) : null;
          
          setTrackData({
            branch: savedBranch || branchOptions[0]?.code,
            activeTab: savedTab || 'overview',
            prefs: savedPrefs
          });

          if (savedBranch && branches[savedBranch]) setBranch(savedBranch);
          if (savedTab) setActiveTab(savedTab);
        }
      } catch (err) {
        console.error("Failed to load track state:", err);
    ...
    fetchTrackState();
  }, [storagePrefix, uid, branches]);

  // Sync track state back to DB when it changes
  useEffect(() => {
    ...
    const syncState = async () => {
      try {
        ...
        await axios.post('http://localhost:8000/api/study-progress/save', {
          track: storagePrefix,
          progress_data
        }, {
          headers: { Authorization: `Bearer ${token}` }
        ...
        // Mirror to local for fallback
        localStorage.setItem(`${storagePrefix}_branch_${uid}`, branch);
        localStorage.setItem(`${storagePrefix}_tab_${uid}`, activeTab);
        localStorage.setItem(storageKey(uid, storagePrefix, 'roadmap_prefs'), JSON.stringify(trackData.prefs));
      } catch (err) {
        console.error("Failed to sync track state:", err);
      }
    };

    const timer = setTimeout(syncState, 1000); 
    return () => clearTimeout(timer);
  }, [trackData, branch, activeTab, storagePrefix, uid, loading]);
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 80, gap: 16 }}>
        <Timer size={48} className="spin" color="var(--accent-primary)" />
        <h2 style={{ fontSize: 20, fontWeight: 700 }}>Initializing Command Center...</h2>
        <p style={{ color: 'var(--text-muted)' }}>Synching your preparation data from the cloud.</p>
      </div>
    );
  }
...
export default function HigherStudies() {
  ...
  const [selectedTrack, setSelectedTrack] = useState(() => {
    if (initialTrack) return initialTrack.toUpperCase();
    // Restore last visited track
    const saved = localStorage.getItem('higher_studies_last_track');
    return saved || null;
  });

  // Sync URL query param + persist last track when it changes
  useEffect(() => {
    if (selectedTrack) {
      setSearchParams({ track: selectedTrack }, { replace: true });
      localStorage.setItem('higher_studies_last_track', selectedTrack);
    } else {
      setSearchParams({}, { replace: true });
      localStorage.removeItem('higher_studies_last_track');
    }
  }, [selectedTrack, setSearchParams]);
